...
//    std::cout <<"\n" << candidates_.at(m)->getId() << " " << candidates_.at(m)->getName();
  }
  
//  std::cout << "\nLosers";
  for (int n = numSeats_; n < (int)(candidates_.size()); n++){
    losers_.push_back(candidates_.at(n));
//    std::cout <<"\n" << candidates_.at(n)->getId() << " " << candidates_.at(n)->getName();

  }

//  std::cout<< "Sort Candidates";
  // wirte audit file
  ...
  if(candidates_.empty())
    ...
    auto dec = [](Candidate* c1, Candidate* c2) {
      if (c1->getBallotsForSize() > c2->getBallotsForSize())
        return 1;
      // when comes to ties situation, randomly pick a candidate
      //else if(c1->getBallotsForSize() == c2->getBallotsForSize())
      // return rand()%2;
      else return 0;
  };
